...
export async function generateMetadata({ params }: PageProps) {
  return blogPostMetadata(params, {
    metaKey: "blogNeedleHangDepth",
    path: "/blog/needle-hang-depth",
    keywords: [...NEEDLE_HANG_KEYWORDS],
  });
}

export default async function NeedleHangDepthPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const tMeta = await getTranslations({ locale, namespace: "meta" });
  const headline = tMeta("blogNeedleHangDepth.title");
  const description = tMeta("blogNeedleHangDepth.description");
  const pageUrl = `${SITE_URL}/${locale}${outwardPath(locale, "/blog/needle-hang-depth")}`;
  ...
  if (locale === "es") {
    return (
      <>
        ...
        />
        <NeedleHangEsArticle headline={headline} backBlog={tNav("backBlog")} />
      </>
    );
  }
